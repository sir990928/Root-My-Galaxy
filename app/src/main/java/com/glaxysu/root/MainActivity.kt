...
import com.glaxysu.root.ui.theme.RootMyGalaxyTheme
...
class MainActivity : ComponentActivity() {
    ...
    private var accentColor by mutableStateOf(AccentColor.Glaxy)
    private var themeMode by mutableStateOf(AppThemeMode.System)
    private var advancedMode by mutableStateOf(false)
    private var rootManager by mutableStateOf(RootManager.KernelSU)
...
private fun managerPackage(manager: RootManager): String = when (manager) {
    ...
    RootManager.SukiSU -> SUKI_SU_MANAGER_PACKAGE
}

private fun managerDownloadUrl(manager: RootManager): String = when (manager) {
    RootManager.KernelSU -> KERNEL_SU_MANAGER_URL
    RootManager.SukiSU -> SUKI_SU_MANAGER_URL
}

private fun isRootManagerInstalled(context: Context, manager: RootManager): Boolean =
    context.packageManager.getLaunchIntentForPackage(managerPackage(manager)) != null

private fun openRootManager(context: Context, manager: RootManager) {
    val launch = context.packageManager.getLaunchIntentForPackage(managerPackage(manager))
    if (launch != null) {
        context.startActivity(launch)
    } else {
        context.startActivity(Intent(Intent.ACTION_VIEW, Uri.parse(managerDownloadUrl(manager))))
    }
}
...
private fun RootApp(
    ...
    onRootManagerChanged: (RootManager) -> Unit,
    ...
    var selectedRootManager by remember { mutableStateOf(rootManager) }
    ...
    if (showInstallConfirmation) {
        AlertDialog(
            ...
            title = {
                ...
                Text(
                    stringResource(
                        R.string.root_manager_confirm_title,
                        stringResource(selectedRootManager.labelRes),
                    ),
                )
            },
            text = {
                Column(verticalArrangement = Arrangement.spacedBy(14.dp)) {
                    Text(stringResource(R.string.root_manager_confirm_body))
                    RootManagerSelector(
                        selectedManager = selectedRootManager,
                        onSelected = { selectedRootManager = it },
                    )
                }
            },
            confirmButton = {
                FilledTonalButton(onClick = {
                    ...
                    onRootManagerChanged(selectedRootManager)
        ...
        AnimatedContent(targetState = selectedPage, label = "page") { page ->
            when (page) {
                AppPage.Overview -> OverviewPage(
                    ...
                    onInstall = {
                        selectedProfile = null
                        selectedRootManager = rootManager
                        if (advancedMode) {
                            showTargetPicker = true
...
@Composable
private fun RootManagerSelector(
    selectedManager: RootManager,
    onSelected: (RootManager) -> Unit,
) {
    Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
        Text(
            text = stringResource(R.string.root_manager_select_title),
        ...
        Column(
            ...
            RootManager.entries.forEach { manager ->
                val selected = manager == selectedManager
                Row(
                    modifier = Modifier
                        .fillMaxWidth()
                        .selectable(
                    ...
                    Column {
                        Text(
                            text = stringResource(manager.labelRes),
                            style = MaterialTheme.typography.titleSmall,
                        )
                        Text(
                            text = stringResource(manager.descriptionRes),
...
private fun OverviewPage(
    ...
    LazyColumn(
        ...
        item { InstallStatusCard(installState, rootManager, onInstall) }
...
private fun InstallStatusCard(
    ...
    val managerInstalled = remember(installState, rootManager) {
        isRootManagerInstalled(context, rootManager)
    }
    Card(
        onClick = {
            when {
                ...
                installState.phase == InstallPhase.Installed -> {
                    if (managerInstalled) {
                        openRootManager(context, rootManager)
                    } else {
                        uriHandler.openUri(managerDownloadUrl(rootManager))
                    }
                }
                else -> onInstall()
            }
        ...
        colors = CardDefaults.cardColors(
            containerColor = if (installState.phase == InstallPhase.Failed) {
        ...
        Row(
            ...
            when {
                installState.busy -> LoadingIndicator(
                    ...
                    color = if (installState.phase == InstallPhase.Failed) {
                        MaterialTheme.colorScheme.onErrorContainer
                    } else {
                        MaterialTheme.colorScheme.onPrimary
                    },
                ...
                if (installState.phase == InstallPhase.Installed) {
                    Row(
                        ...
                        Icon(
                            ...
                            tint = if (installState.phase == InstallPhase.Failed) {
                                MaterialTheme.colorScheme.onErrorContainer
                            } else {
                                MaterialTheme.colorScheme.onPrimary
                            },
                        ...
                        Text(
                            text = stringResource(
                                ...
                                stringResource(rootManager.labelRes),
                            ),
                            style = MaterialTheme.typography.titleMedium,
                        )
                    }
                ...
                Text(
                    text = when (installState.phase) {
                        InstallPhase.Installed -> stringResource(
                            if (managerInstalled) {
                                R.string.install_tap_open_manager_format
                            } else {
                                R.string.install_tap_manager_format
                            },
                            stringResource(rootManager.labelRes),
                        )
                        InstallPhase.Failed -> stringResource(R.string.install_tap_retry)
                        else -> stringResource(
                            R.string.install_tap_start_format,
                            stringResource(rootManager.labelRes),
                        )
                    },
...
private fun SettingsPage(
    ...
    var showWirelessDebuggingDialog by remember { mutableStateOf(false) }
    var wirelessConnected by remember { mutableStateOf(false) }
    var wirelessPaired by remember { mutableStateOf(AppPreferences.wirelessAdbPaired(context)) }
    ...
    val wirelessStatusScope = rememberCoroutineScope()
    val notificationPermissionLauncher = rememberLauncherForActivityResult(
        ActivityResultContracts.RequestPermission(),
    ) {
        WirelessAdbNotifications.showPairingPrompt(context)
    ...
    if (showWirelessDebuggingDialog) {
        WirelessDebuggingDialog(
            onDismiss = { showWirelessDebuggingDialog = false },
            onOpenSettings = {
                showWirelessDebuggingDialog = false
                if (
    ...
    LazyColumn(
        ...
        item { SectionLabel(stringResource(R.string.connection)) }
        item {
            SettingsCard(
                icon = Icons.Rounded.Wifi,
                title = stringResource(R.string.wireless_debugging),
                description = if (wirelessPaired) {
                    ""
                } else {
                    stringResource(R.string.wireless_debugging_first_time)
                },
                trailingContent = {
                    Switch(
                        checked = wirelessConnected,
                    ...
                    wirelessStatusScope.launch {
                        ...
                        if (!connected) {
                            if (paired) {
                                ...
                                showWirelessDebuggingDialog = true
                            }
                        }
                    }
                },
...
@Composable
private fun WirelessDebuggingDialog(
    onDismiss: () -> Unit,
    onOpenSettings: () -> Unit,
) {
    AlertDialog(
        ...
        title = {
            DialogDimAmount(0.24f)
            Text(stringResource(R.string.wireless_debugging_title))
        },
        text = {
            Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
                Text(stringResource(R.string.wireless_debugging_body))
                Text(
                    stringResource(R.string.wireless_debugging_notification_body),
                    color = MaterialTheme.colorScheme.onSurfaceVariant,
                    style = MaterialTheme.typography.bodyMedium,
                )
            }
        ...
        confirmButton = {
            Button(
                ...
                Text(stringResource(R.string.wireless_debugging_open_settings))
            }
        },
        dismissButton = {
...
private fun openWirelessDebuggingSettings(context: Context) {
    val wirelessIntent = Intent("android.settings.APPLICATION_DEVELOPMENT_SETTINGS")
    runCatching {
...
private fun accentLabel(color: AccentColor): String = when (color) {
    ...
    AccentColor.Glaxy -> stringResource(R.string.color_glaxy)
